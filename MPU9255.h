...
public:
  ...
  uint8_t mxl=0;//X axis
  uint8_t myl=0;//Y axis
  uint8_t mzl=0;//Z axis
  ...
  uint8_t dor_status=0;
  uint8_t hofl_status=0;
//liczby do ustawienia czułości/offsetu
  uint8_t asax=177;
  uint8_t asay=179;
  uint8_t asaz=167;
//obliczone wartości ze wzoru na offset (((asax-128)*0.5)/128 +1) )
  float off_asax=1.19;
  float off_asay=1.20;
  float off_asaz=1.15;
  ...
  uint8_t testIMU();//test gyroscope and accelerometer(InertialMeasurementUnit)
  ...
  uint8_t read_mag();//read data from the magnetometer
  int8_t self_test();//func for testing magnetometer 
  void set_sensitivity();
  int16_t read_temp();//read temperature
  uint8_t get_info(uint8_t address, uint8_t subAddress);
  float total_mag_vector(float x, float y, float z);
  ...
  private:
  void requestBytes(uint8_t address, uint8_t subAddress, uint8_t bytes, uint8_t *data);//request data
  
  void readArray(uint8_t *output, char size, uint8_t data_in);//read an array of bytes
